...
import { emptyTrip } from './AdminTrips'

const EditTrip = ({ activeTrip, setActiveTrip, newTrip, setNewTrip }) => {
  ...
  const handleSubmit = (e) => {
    ...
    axios
      .post('/admin/trips', activeTrip, {
        auth: { username: username, password: password },
      })
      .then((response) => (response.status === 200 ? back() : setInvalid(true)))
      .catch(() => setInvalid(true))
      .finally(mutate('/trips'))
  }
  ...
  return (
    <Form onSubmit={(e) => handleSubmit(e)} className="mt-3">
      ...
      <div style={{ display: 'flex', gap: '1rem', justifyContent: 'center' }}>
        <Button onClick={() => back()} variant="warning">
          Cancel
        </Button>
